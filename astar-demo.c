...
void enqueue2(Queue * Q, int x, int y, int endptx, int endpty){
	QueueNode * alpha;
	...
	if (alpha == NULL) queueOverflow();
		...
		if (Q->front == NULL){
			...
			AlphaDistance = distanceFormula(x, y, endptx, endpty);
			CurrentDistance = distanceFormula(Q->front->coord[0], Q->front->coord[1], endptx, endpty);
...
Queue Neighbors_AStar(int array[][COL], int camefrom[][COL], int x, int y, int endx, int endy){
	Queue Neighbor;
	initQueue(&Neighbor);

	if (isInsideGrid(array, x, y-1) && isPassable(array, x, y-1) && camefrom[y-1][x] == SPC){
		enqueue2(&Neighbor, x, y-1, endx, endy);
		camefrom[y-1][x] = NTH;
	}
	if (isInsideGrid(array, x-1, y) && isPassable(array, x-1, y) && camefrom[y][x-1] == SPC){
		enqueue2(&Neighbor, x-1, y, endx, endy);
		camefrom[y][x-1] = WST;
	}
	if (isInsideGrid(array, x, y+1) && isPassable(array, x, y+1) && camefrom[y+1][x] == SPC){
		enqueue2(&Neighbor, x, y+1, endx, endy);
...
void AStar(int array[][COL], int camefrom[][COL], int start[], int end[]){
	...
	enqueue2(&Q, start[0], start[1], end[0], end[1]);													//Queue in the starting point
	visitedArray[start[1]][start[0]] = 1;												//Mark it as visited
	...
	while (!isEmptyQueue(&Q)){															//While the queue is not empty
		...
		nb = Neighbors_AStar(array, camefrom, dequeued[0], dequeued[1], end[0], end[1]);
		...
		while (!isEmptyQueue(&nb)){														//Now that we have the sorted nb, dequeue them then visit!
			dequeuedNeighbor = dequeue(&nb);
			if (visitedArray[dequeuedNeighbor[1]][dequeuedNeighbor[0]] == 0){			//Visit nodes if they haven't been visited though. Otherwise, skip
				enqueue2(&Q, dequeuedNeighbor[0], dequeuedNeighbor[1], end[0], end[1]);				
			}
		}
	}

	enqueue2(&pathtrace, end[0], end[1], end[0], end[1]);
	
	while (!(pathtrace.rear->coord[0] == start[0] && pathtrace.rear->coord[1] == start[1]) && !isEmptyQueue(&Q)){
		switch(camefrom[pathtrace.rear->coord[1]][pathtrace.rear->coord[0]]){
			case NTH:
				enqueue2(&pathtrace, pathtrace.rear->coord[0], pathtrace.rear->coord[1]+1, end[0], end[1]);				
				break;
			case WST:
				enqueue2(&pathtrace, pathtrace.rear->coord[0]+1, pathtrace.rear->coord[1], end[0], end[1]);				
				break;
			case STH:
				enqueue2(&pathtrace, pathtrace.rear->coord[0], pathtrace.rear->coord[1]-1, end[0], end[1]);
				break;
...
int main(){
	...
	AStar(maze, gridBFS, startpt, endpt);
